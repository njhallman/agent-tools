...
$OVERLEAF_PROJECT_ID, else a `project-id` file in the skill directory. There is
no built-in default: a wrong project id would apply this repo's edits to some
...
id is included -- set OVERLEAF_PROJECT_ID or write the skill's `project-id`
file, or the client will refuse to run rather than guess.
"""
import base64
import json
import pathlib
...
print(
    ...
    "logged-in browser)\\nand set the project id, e.g.\\n"
    f"  echo <24-hex-id-from-the-project-URL> > {{root / 'project-id'}}\\n"
